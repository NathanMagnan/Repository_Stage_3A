...
for i in range(42):
    ...
    if (i != 41):
        ...
        Patchy = pandas.read_csv(path, sep = ' ', names = ['X', 'Y', 'Z', 'vX', 'vY', 'vZ', 'vMax'])
        X = Patchy['X'].values
        Y = Patchy['Y'].values
        Z = Patchy['Z'].values
        ...
        # extracting the MST statistics and renormalizing them
        X_d = patchy_histogram['x_d']
        Y_d = patchy_histogram['y_d'] / np.max(patchy_histogram['y_d']) # we normalize to 1 to avoid probleme with different box sizes
        
        X_l = patchy_histogram['x_l']
        Y_l = patchy_histogram['y_l'] / np.max(patchy_histogram['y_l']) # we normalize to 1 to avoid probleme with different box sizes
        
        X_b = patchy_histogram['x_b']
        Y_b = patchy_histogram['y_b'] / np.max(patchy_histogram['y_b']) # we normalize to 1 to avoid probleme with different box sizes
        
        X_s = patchy_histogram['x_s']
        Y_s = patchy_histogram['y_s'] / np.max(patchy_histogram['y_s']) # we normalize to 1 to avoid probleme with different box sizes
    ...
    np.savetxt(str(target) + str(i) + "_X_b", X_s)
    np.savetxt(str(target) + str(i) + "_Y_b", Y_s)
    ...
    Y_s_kept = Y_s_kept[::3]
    X_s_kept = np.log10(X_s_kept) # we normalize X axis between 0 and 1
