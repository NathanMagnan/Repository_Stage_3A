...
for i in range(4):
    for j in range(4):
        for k in range(4):
            ...
            Y_d_estimator_new = np.array([0 for n in range(np.shape(X_d_estimator)[0])]) # we will these with the values of the estimator
            Y_l_estimator_new = np.array([0 for n in range(np.shape(X_l_estimator)[0])])
            Y_b_estimator_new = np.array([0 for n in range(np.shape(X_b_estimator)[0])])
            Y_s_estimator_new = np.array([0 for n in range(np.shape(X_s_estimator)[0])])
            ...
            if (number == 0):
                ...
                Y_d_std_estimator = np.sqrt((number * (Y_d_estimator_old**2 + Y_d_std_estimator_old**2) + Y_d_std_estimator_new.copy()**2) / (number + 1) - Y_d_estimator.copy())
            ...
            if (number == 0):
                ...
                Y_l_std_estimator = np.sqrt((number * (Y_l_estimator_old**2 + Y_l_std_estimator_old**2) + Y_l_std_estimator_new.copy()**2) / (number + 1) - Y_l_estimator.copy())
            ...
            if (number == 0):
                ...
                Y_b_std_estimator = np.sqrt((number * (Y_b_estimator_old**2 + Y_b_std_estimator_old**2) + Y_b_std_estimator_new.copy()**2) / (number + 1) - Y_b_estimator.copy())
            ...
            if (number == 0):
                ...
                Y_s_std_estimator = np.sqrt((number * (Y_s_estimator_old**2 + Y_s_std_estimator_old**2) + Y_s_std_estimator_new.copy()**2) / (number + 1) - Y_s_estimator.copy())
