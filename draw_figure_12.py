...
# Coordinates
H0 = np.linspace(60, 75, 250)
W0 = np.linspace((-1.40), (-0.60), 250)
Ns = np.linspace(0.920, 0.995, 250)
Sigma8 = np.linspace(0.64, 1.04, 250)
OmegaM = np.linspace(0.250, 0.375, 250)
...
for i in range(n_grids):
    ...
    for a in range(n_points):
        ...
        X_new = [h0, w0, ns, sigma8, omegaM]
        
        Y_predicted, Cov = gp.compute_prediction(X_new = X_new)
        
        chi2 = GP.chi_2(Y_model = Y_planck, noise_model = noise, Y_observation = Y_predicted, Noise_observation = Cov)
...
for i in range(5):
    for j in range(i+1):
        ...
        if (j == i):
            ...
            subplot.plot(Grids['grid_white'][k][:, 0], Grids['grid_white'][k][:, 1], 'r')
            subplot.plot(Grids['grid_blue'][k][:, 0], Grids['grid_blue'][k][:, 1], 'g')
            ...
            subplot.scatter(Grids['grid_white'][k][:, 1], Grids['grid_white'][k][:, 0], s = 0.001, c = 'red')
            subplot.scatter(Grids['grid_blue'][k][:, 1], Grids['grid_blue'][k][:, 0], s = 0.001, c = 'green')
...
my_file = 'Sensitivity_MST_Abacus.png'
