...
X_data = np.loadtxt(str(target) + "_X_data_d") # numpy array with fields h0, w0, ns, sigma8, omegaM, d -- 6 points per simu
...
X_planck = X_data[240:246]
Y_planck = Y_data[240:246]

h0_i, w0_i, ns_i, sigma8_i, omegaM_i = X_planck[5, 0:5]
...
X_training = X_data[0:240]
Y_training = Y_data[0:240]
...
gp.initialise_model()
# add a line here if we found that a prior or bounds gives better results
print(gp.model)
...
gp.optimize_model() # modify these line of we found that 'gcs' or optmize_restart gives better results
...
# Grids
Grids = {'name_x_axis' : [], 'name_y_axis' : [], 'unit_x_axis' : [], 'unit_y_axis' : [], 'x_axis' : [], 'y_axis' : [], 'grid' : [], 'chi2_grid' : [], 'grid_green' : [], 'grid_red' : []}
...
for i in range(5):
    for j in range(i+1):
        k = K[i][j]
        if (j == i):
            if (Grids['grid_green'][k] != []):
                np.savetxt(str(target) + "_grid_" + str(k) + "_X_green_d", Grids['grid_green'][k][:, 0])
                np.savetxt(str(target) + "_grid_" + str(k) + "_Y_green_d", Grids['grid_green'][k][:, 1])
            if (Grids['grid_red'][k] != []):
                np.savetxt(str(target) + "_grid_" + str(k) + "_X_red_d", Grids['grid_red'][k][:, 0])
                np.savetxt(str(target) + "_grid_" + str(k) + "_Y_red_d", Grids['grid_red'][k][:, 1])
        else:
            if (Grids['grid_green'][k] != []):
                np.savetxt(str(target) + "_grid_" + str(k) + "_X_green_d", Grids['grid_green'][k][:, 0])
                np.savetxt(str(target) + "_grid_" + str(k) + "_Y_green_d", Grids['grid_green'][k][:, 1])
