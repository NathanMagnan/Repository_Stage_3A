...
for i in range(n_groups):
   ...
   List_groups.append((X_l_data_a, Y_l_data_a, X_l_test_a, Y_l_test_a))
...
for i in range(len(Kernel_names)):
    ...
    for j in range(n_groups):
        ...
        gp = GP.GP(X = X_data, Y = Y_data, n_points_per_simu = 5, Noise = None, make_covariance_matrix = False)
...
for i in range(len(Kernel_names)):
    print("Covariance matrix for statistic l and kernel " + Kernel_names[i] + " :")
...
plt.title("$k$-fold cross validation : $l$")
...
## showing that cov does not output the right covariance matrix
# """ the reasons is that it supresses the average, but we don't want that here"""
# print("computing the new covariance matrix")
# 
# errors = np.asarray(Errors[i]).T
# 
# cov_new = [[0 for i in range(np.shape(errors)[0])] for j in range(np.shape(errors)[0])]
...
# cov_new = np.asarray(cov_new)
# 
# cov_old = np.cov(errors)
# 
# print("old covariance matrix :")
# print(cov_old)
# print("new covariance matrix :")
# print(cov_new)
# 
# print("new covariance matrix computed")