...
for i in range(5):
    for j in range(i+1):
        ...
        try:
            Grid_green_X = np.loadtxt(str(target) + "_grid_" + str(k) + "_X_green_d")
        except:
            Grid_green_X = []
        try:
            Grid_green_Y = np.loadtxt(str(target) + "_grid_" + str(k) + "_Y_green_d")
        except:
            Grid_green_Y = []
        try:
            Grid_red_X = np.loadtxt(str(target) + "_grid_" + str(k) + "_X_red_d")
        except:
            Grid_red_X = []
        try:
            Grid_red_Y = np.loadtxt(str(target) + "_grid_" + str(k) + "_Y_red_d")
        except:
            Grid_red_Y = []
        ...
        if (j == i):
            ...
            if (Grid_green_X != []):
                subplot.plot(Grid_green_X, Grid_green_Y, 'r')
            if (Grid_red_X != []):
                subplot.plot(Grid_red_X, Grid_red_Y, 'g')
            ...
            if (Grid_green_X != []):
                subplot.scatter(Grid_green_X, Grid_green_Y, s = 1, c = 'red')
            if (Grid_red_X != []):
                subplot.scatter(Grid_red_X, Grid_red_Y, s = 1, c = 'green')
...
plt.suptitle("Sensitivity of the degree $d$ to cosmological parameters")
print("results plotted")

print("starting to save the results")
my_path = os.path.abspath('/home/astro/magnan/Repository_Stage_3A/Figures')
my_file = 'Sensitivity_MST_Abacus_d'
